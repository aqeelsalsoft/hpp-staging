    ...
    <div class="search__innerWrap relative w-full xl:w-[460px]">
      <UInputMenu @change="onItemSelected(selected)" v-model="selected" :search="search" :loading="loading"
        ...
        <template #trailing-icon>
          <button aria-label="Expand or collapse options" type="button" tabindex="-1" aria-haspopup="listbox" aria-expanded="false" data-headlessui-state="" class="absolute inset-y-0 end-0 flex items-center px-2.5">
          <i class="i-heroicons-chevron-up-down-20-solid" aria-label="Expand or collapse options" />
          </button>
        </template>
      </UInputMenu>
    </div>
