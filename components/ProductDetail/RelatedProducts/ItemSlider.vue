...
<template>
    <div class="hpp__industryCarouselWrapper relative overflow-hidden">
        <div ref="slider" class="keen-slider !hidden sm:!flex !overflow-visible overflow-y-hidden grow-0">
            <div class="keen-slider__slide !overflow-visible" v-for="(item, index) in props.products" :key="index">
                    ...
                    <NuxtLink :to="`/${item.primary_industry}/${item.slug}`"
                        ...
                        <div class="thumb__wrap w-[100%] bg-[#efefef] rounded-[32px] overflow-hidden">
                            <NuxtImg format="webp" :src="`${useRuntimeConfig().public.productMediaURL}${item.feature_image}`"
                                :alt="item.feature_image_alt" loading="lazy" fit="cover" width="336" height="336"
                                class="w-full h-auto" />
                        </div>
                        <div class="desc__wrap py-[10px]">
                            <div
                                ...
                                <span>Min. {{item.max_order_quantity > 0 ? item.max_order_quantity : 200}} units</span><span class="text-[18px] px-2">·</span><span>Delivery: 4
                                    weeks</span>
                            </div>
                            <h3 class="font-headings text-[20px] leading-[24px] text-acai font-bold mb-0.5">{{item.name}}</h3>
